...
function DashboardContent() {
  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-12 px-4 py-12 sm:px-6 md:flex-row md:gap-8 md:p-8">
        ...
        <nav className="flex gap-2 overflow-x-auto md:flex-col md:overflow-visible">
          {SIDEBAR_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="whitespace-nowrap rounded-full px-3 py-2 text-sm font-medium text-olympiad-800 transition-colors hover:bg-black/5"
            >
              {link.label}
            </Link>
          ))}
