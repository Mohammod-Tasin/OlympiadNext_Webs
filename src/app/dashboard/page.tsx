  ...
  return (
    <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 py-8 sm:px-6 md:flex-row">
      <aside className="w-full shrink-0 md:w-56">
        <Card className="p-4 md:sticky md:top-24">
          <nav className="flex gap-2 overflow-x-auto md:flex-col md:overflow-visible">
            {SIDEBAR_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="whitespace-nowrap rounded-full px-3 py-2 text-sm font-medium text-olympiad-800 transition-colors hover:bg-white/60"
              >
                {link.label}
              </Link>
            ))}
